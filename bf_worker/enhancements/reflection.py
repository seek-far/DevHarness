...
def _apply_crash_note(state: dict) -> str:
    ...
    return (
        ...
        "The patcher is ANCHORED ON CONTENT: it replaces the line whose "
        "current text equals `original_line` (using `line_number` only as a "
        "hint). It rejects the fix when `original_line` is empty or is not "
        "the verbatim current text of an existing line.\n\n"
        "What went wrong:\n" + err_block + "\n\n"
        "Rules for the next attempt:\n"
        "1. Set `original_line` to the EXACT current text of ONE existing "
        "line to anchor on — copy it verbatim from the AUTHORITATIVE numbered "
        "current-file view in this prompt (a prior attempt may have already "
        "changed that line; use its current text, not the original source). "
        "Never use an empty `original_line`.\n"
        "2. Set `line_number` to that line's 1-based number in the current "
        "file.\n"
        "3. To ADD code, set `new_line` to MULTIPLE lines (real newlines) "
        "that include the anchored original line plus the additions, each "
        "properly indented — do NOT cram statements onto one line with `;`.\n"
